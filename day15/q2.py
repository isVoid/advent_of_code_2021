...
for iy, ix in np.ndindex(arr.shape):
    tiley = iy // original_shape[1]
    tilex = ix // original_shape[0]
    arr[iy, ix] = (arr[iy, ix] - 1 + tilex + tiley) % 9 + 1

# np.savetxt("checkarr.txt", arr, fmt="%i", delimiter=" ")
...
print(risk[P(arr.shape[0] - 1, arr.shape[1] - 1)])
